...
上传入口仅接受 PNG 和 JPEG。插件会在后台读取图片尺寸元数据，拒绝超过 2048 像素边长或 4,194,304 像素总量的图片；通过校验的图像会重新编码为 PNG 后才交给 Halo 附件服务保存。因此，原文件中的 EXIF、其他元数据和尾随字节不会随附件保存。

单张图片同时受管理员设置与 10 MiB 服务端硬上限约束。每个插件实例同一时间最多处理两张上传图片，超过该限制时会返回“上传繁忙”而不是继续在内存中排队。当前实现仍会在受限并发下将单个合格文件读入内存以完成安全解码和重编码。
...
- 用户输入、AI 回复、AI 思考过程、会话标题、摘要、记忆、标签、收藏状态、消息编辑内容，以及每条 AI 回复实际使用的模型和任务类型。
- 上传图片的文件名、媒体类型、大小、Halo 附件引用；图像生成结果会先由插件后端限制大小、校验并保存到 Halo 附件库，再以附件链接写入会话。
...
浏览器本地缓存仅用于短暂的离线兜底，且最多保存最近更新的 50 个会话。缓存键会按当前登录用户标识隔离；在身份尚未确认时插件不会读取任何缓存。服务端明确返回空会话时会清除没有待同步修改的缓存。页面隐藏或组件卸载时，插件会按会话 ID、所属用户和随机确认 token 保留待同步快照，只有同一用户的对应 token 获得服务端确认后才清除，以避免旧请求或其他标签页覆盖最后一次编辑。切换账号时新账号只会读取自己的缓存，旧账号待同步快照不会提交给新账号；短暂的身份检查失败不会删除本地草稿或待同步记录，只有已确认的账号切换才会清理旧账号的内存视图。
...
- 在聊天侧边栏删除会话，插件会删除该会话的新格式快照，并清理旧格式中的对应会话记录。为阻止已经发出但较晚完成的旧浏览器请求重新创建会话，插件会在原会话 ConfigMap 中仅保留会话版本和删除时间的删除标记，最长 30 天后由清理任务删除；该标记不包含消息、附件或会话正文。
...
禁用、重载或卸载插件时，插件会停止自身的心跳、恢复和清理调度器，并中断当前实例持有的运行任务，将可保存的任务标记为 `interrupted`。该操作不会主动删除聊天、审计、Job、设置等 ConfigMap，也不会删除 Halo 附件；这用于避免误删数据。AI Foundation 已经收到的请求是否继续处理，取决于 AI Foundation 和第三方服务商。

## 上下文摘要与原始记录

聊天界面向模型发送的上下文会限制为近期消息、用户保存的长期记忆和可选摘要，以控制请求大小。手动“压缩上下文”只会写入记忆摘要，不会删除、替换或隐藏原始消息；自动上下文控制同样不会改写已保存的会话记录。用户仍可在会话中查看、导出或删除原始消息。

自动标题、手动摘要和上下文压缩仅在本次对话最近一次 AI 回复实际使用的语言或多模态模型仍可用时执行，且不会把 AI 思考过程作为辅助调用输入；图像生成不会触发这些辅助调用。图像生成结果只有以受限 `data:image` 形式保存在会话中时才会作为后续多模态上下文传递，插件不会把任意远程图片 URL 自动转交给模型。为避免读取 AI 回复时触发第三方追踪，Markdown 和历史图像只会加载 Halo 同源附件链接或受限 `data:image` 数据。
