...
Halo AI Console 是社区维护的 Halo Console 插件，不是 Halo 官方产品。本说明描述插件 `0.2.5` 的实际数据处理方式。Halo 站点管理员负责决定是否启用插件、配置哪些 AI Foundation 模型、授予哪些权限以及设置保存期限。
...
- 会话、消息、摘要、个人设置和图片缓存：当前没有自动过期策略，会一直保留到用户删除、管理员清理或对应 ConfigMap 被删除。
...
- 该操作不会自动删除与会话相关的 Job、调用日志、图片缓存或 Halo 附件；Job 和日志按上述保留策略清理，图片缓存目前没有自动过期任务。
- 普通用户不能删除审计日志，插件没有给普通用户提供审计日志删除接口。
- Halo 附件需要由有权限的用户在 Halo 附件管理中单独删除。

当前版本没有“删除我的全部数据”按钮。若用户需要完整删除个人数据，应联系 Halo 站点管理员，由管理员在确认用户标识后清理该用户的 `store`、`session`、`job`、`log`、`usage` ConfigMap 及历史遗留对象；不要删除全局 `halo-ai-console-config`，否则会影响所有用户的管理员配置。历史迁移遇到旧扩展类型索引缺失时，旧对象可能被保留，管理员还需要按迁移结果中的警告继续处理。

## 用户如何导出数据

当前版本没有完整的个人数据导出接口。聊天页的导出操作只导出单条 AI 回复的 Markdown 文件，不包含完整会话、个人设置、调用日志、Job、图片缓存或 Halo 附件。需要完整副本时，应由管理员从 Halo 数据存储和附件管理中按用户范围导出，并同时注意其中包含的个人信息和第三方模型输入。
...
拥有聊天权限的用户可以查看自己的调用记录；拥有单独的审计权限的用户可以打开自己的审计视图；拥有管理员/全部日志权限的用户可以查看全部用户的审计日志。普通用户不能删除审计日志。日志会在默认 90 天或管理员配置的期限后由清理任务删除，但清理任务不保证已经导出的副本或第三方模型服务商处的日志同步删除。
