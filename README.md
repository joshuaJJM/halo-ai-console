...
```text
halo-ai-console-0.4.1-alpha.15.jar
```
...
2. 安装测试包 `halo-ai-console-0.4.1-alpha.15.jar`。从 `0.3.5` 热升级后，请在 Halo 插件页执行一次"重载插件"；也可以重启 Halo，以刷新后端组件和前端资源。旧页面必须刷新后才能保存会话。该 alpha 版本仅用于测试，不应覆盖任何已发布制品。
